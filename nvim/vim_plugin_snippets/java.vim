...
" src 为classpath 资源路径 out 为编译输出路径
autocmd FileType java nnoremap <leader>r :w<ESC>:!javac -cp src % -d out<Enter><ESC>
			\:sp \| resize 12 \| terminal w=%:.:r;c=${w\#*/};time java -cp out $c<Enter>G

autocmd FileType java inoremap <leader>r <ESC>:w<ESC>:!javac -cp src % -d out<Enter><ESC>
			\:sp \| resize 12 \| terminal w=%:.:r;c=${w\#*/};time java -cp out $c<Enter>G
